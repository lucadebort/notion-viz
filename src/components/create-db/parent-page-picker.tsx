  ...
  if (value) {
    return (
      <button
        ...
        className="flex items-center gap-2 w-full px-2 h-8 rounded-md border border-border bg-muted form-text text-foreground cursor-pointer hover:bg-muted/80 transition-colors"
      >
        {value.icon && <span>{value.icon}</span>}
        <span className="flex-1 text-left truncate">{value.title}</span>
        <span className="text-muted-foreground sub-config-text">change</span>
      </button>
    )
  }
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-md border border-border bg-popover shadow-md overflow-hidden">
          {loading && (
            <p className="px-3 py-2 form-text text-muted-foreground">Searching…</p>
          )}
          {!loading && results.length === 0 && (
            <p className="px-3 py-2 form-text text-muted-foreground">No pages found</p>
          )}
          {results.map((page) => (
            <button
              key={page.id}
              ...
              className="flex items-center gap-2 w-full px-3 py-2 form-text hover:bg-muted transition-colors cursor-pointer"
            >
